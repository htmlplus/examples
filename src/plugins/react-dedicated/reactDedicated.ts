...
export interface IReactDedicatedOptions {
        ...
        element: {
          attribute: {
            ...
            default(parameters) {
              ...
              const value = getAttribute(parameters.element, name);

              // it should be after the `getAttribute`
              parameters.name.name = camelCase(name);
              ...
              if (
                ...
                options.isStringAttribute(parameters.element.openingElement.name.name, name)
              )
                return;
          ...
          default(parameters) {
            const previous = (parameters.pattern.node.openingElement.name as any).name;

            if (!isCustomElement(parameters.pattern.node)) return;

            const next = options.customElementNameResolver(previous);

            parameters.pattern.node.openingElement.name = t.jsxIdentifier(next);

            if (!parameters.pattern.node.closingElement) return;
